    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl p-6 overflow-y-auto max-h-[90vh]">
      ...
      <div v-if="step === 1">
        ...
        <div>
          ...
          <input type="text" v-model="searchQuery" @input="debouncedSearchPatients" id="search" class="input"
            placeholder="Example: Pedro Penduko" />
        </div>

        <!-- Search Results -->
        <div v-if="searchQuery.trim()" class="bg-white border border-gray-300 rounded-lg max-h-60 overflow-y-auto mt-4">
      ...
      <div v-if="step === 3">
        ...
        <div class="mb-4">
          ...
          <select v-model="form.vaccineCategory" id="vaccineCategory" class="input">
            ...
            <option v-for="category in filteredVaccineCategories" :key="category" :value="category">
              {{ category }}
            </option>
          </select>
      ...
      <div v-if="step === 4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="number" id="age" v-model="computedVaccinationAge" class="input"
              :placeholder="isUnderOneYear ? 'Enter age in months' : 'Enter age in years'"
              :value="computedVaccinationAge" readonly />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      step: 1,
      searchQuery: "",
      filteredPatients: this.patients || [],  // Initialize as empty array
  ...
  watch: {
    'form.birthdate': function () {
      if (this.form.birthdate) {
        ...
        this.updateAgeFields(); // Ensures age in months/years is updated
      }
    },
    ...
    patients: {
      ...
      handler(newPatients) {
        if (Array.isArray(newPatients)) {
          ...
          console.warn('Patients array is not valid.');
        }
      },
    },
    searchQuery: {
  ...
  computed: {
    ...
    if (!patient || !patient.birthdate) {
      return categories; // Return an empty list if patient or birthdate is not available
    }

    const age = this.computedAge;

    // Pregnant category available only for female patients
    if (patient.sex === 'Female') {
      categories.push('Pregnant');
    }

    // Under 1 Year category
    if (age !== '' && age < 1) {
      categories.push('Under 1 Year');
    }

    // Senior Citizen category for patients aged 60 and above
    if (age >= 60) {
      categories.push('Senior Citizen');
    }

    // School-aged Immunization category for specific age range (Grade 1-7)
    if (age >= 6 && age <= 12) {
      categories.push('School-aged Immunization');
    }
    ...
    reviewFields() {
      return {
        Name: `${this.form.firstName} ${this.form.lastName} ${this.form.middleName} ${this.form.suffix}`,
        purok: this.form.purok || "N/A",
        barangay: this.form.barangay || "N/A",
        age: this.form.age === 0 ? "Less than 1 year" : this.form.age || "N/A", // Handle less than 1 year
        birthdate: this.form.birthdate || "N/A",
        contact: this.form.contact || "N/A",
        sex: this.form.sex || "N/A",
        vaccineCategory: this.form.vaccineCategory || "N/A",
        vaccineType: this.form.vaccineType || "N/A",
        dateOfVisit: this.form.dateOfVisit || "N/A",
        ageInMonths: this.isUnderOneYear ? `${this.computedVaccinationAge} months` : "N/A", // Use months if "Under 1 Year"
        ageInYears: !this.isUnderOneYear ? `${this.computedVaccinationAge} years` : "N/A", // Use years otherwise
        weight: this.form.weight || "N/A",
        height: this.form.height || "N/A",
        temperature: this.form.temperature || "N/A",
        antigenGiven: this.form.antigenGiven || "N/A",
        injectedBy: this.form.injectedBy || "N/A",
        exclusivelyBreastfed: this.form.exclusivelyBreastfed || "N/A",
        nextAppointment: this.form.nextAppointment || "N/A",
      };
    },
    computedAge() {
      ...
      return age < 1 ? 0 : age; // Return 0 for ages under 1 year
    },

    computedVaccinationAge() {
      if (!this.form.birthdate) return ''; // If no birthdate, return empty
      const birthDate = new Date(this.form.birthdate);
      const today = new Date();
      ...
      return this.isUnderOneYear ? totalMonths : Math.floor(totalMonths / 12); // Use months if "Under 1 Year"
    },
  ...
  methods: {
    ...
    searchPatients() {
      ...
      // If the query is empty, reset to the full list of patients
      if (query === '') {
      ...
      this.filteredPatients = this.patients.filter((patient) => {
        // Create a full name string and check if it matches the query
      ...
      // Dynamically update the `allowAddNewPatient` data property (if applicable)
      if ('allowAddNewPatient' in this) {
        this.allowAddNewPatient = query.length > 0 && this.filteredPatients.length === 0;
      }
    },
    ...
    updateAgeFields() {
      ...
      if (this.isUnderOneYear) {
        this.form.ageInMonths = age; // Set age in months for "Under 1 Year"
        this.form.ageInYears = ''; // Clear years
      } else {
        this.form.ageInYears = age; // Set age in years
    ...
    addOrNextStep() {
      if (this.selectedPatient) {
        // Proceed to the next step for existing patients
        this.nextStep();
      } else if (this.allowAddNewPatient) {
        // Dynamically calculate and assign age if birthdate is provided
        if (this.form.birthdate) {
          this.form.age = this.computedAge;
        }

        // Ensure the form resets correctly for a new patient
    ...
    selectPatient(patient) {
      // Store the entire patient object including personalId
      this.selectedPatient = {
        ...patient,
        personalId: patient.personalId // Ensure personalId is included
      };
    ...
    validateStep4() {
      if (!this.selectedPatient && !this.validatePatientInfo()) {
        return false;
      }

      // Additional validation for vaccination details
      this.errors = {};
      let valid = true;
      ...
      if (this.isUnderOneYear && !this.form.exclusivelyBreastfed) {
        this.errors.exclusivelyBreastfed =
          "This field is required for children under 1 year.";
        valid = false;
      }
    ...
    saveVaccination() {
      ...
      const vaccinationData = {
        vaccinationDetails: {
          ...
          exclusivelyBreastfed: this.isUnderOneYear ? this.form.exclusivelyBreastfed : null,
